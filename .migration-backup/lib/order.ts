...
export async function saveOrder(
  ...
  if (supabase) {
    ...
    if (data) existingIds = (data as any[]).map(o => o.case_id).filter((id): id is string => !!id)
  ...
  if (supabase) {
    ...
    const saved = supabase ? await saveOrderToSettings(supabase, order) : false
    if (saved) { saveOrderToLocal(order); return }
  }
  saveOrderToLocal(order)
}

async function saveOrderToSettings(supabase: NonNullable<ReturnType<typeof createClient>>, order: Order): Promise<boolean> {
  try {
...
export async function getSettingsOrders(): Promise<Order[]> {
  ...
  try {
    ...
    const orders: Order[] = data?.value ? JSON.parse(data.value) : []
    let changed = false
    const existing = orders.map(o => o.case_id).filter((id): id is string => !!id)
...
export function getLocalOrders(): Order[] {
  try {
    ...
    let changed = false
    orders = orders.map(o => {
      if (o.status === "جديد") { changed = true; return { ...o, status: "قيد المعالجة", case_id: o.case_id || generateCaseId(orders.filter(x => x.id !== o.id).map(x => x.case_id).filter((x): x is string => !!x)) } }
      if (!o.case_id) { changed = true; return { ...o, case_id: generateCaseId(orders.filter(x => x.id !== o.id).map(x => x.case_id).filter((x): x is string => !!x)) } }
      return o
    })
