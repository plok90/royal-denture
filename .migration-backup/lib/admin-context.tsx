...
export function AdminProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (username: string, password: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      localStorage.setItem("rd_admin_session", JSON.stringify({ token, name: data.name }))
      setIsAdmin(true)
      setCurrentAdmin({ name: data.name, username: data.username })
      setIsMainAdmin(data.username === MAIN_ADMIN_USERNAME)
      return { success: true }
    } catch {
      return { success: false, error: "ÙØ´Ù„ Ø§Ù„Ø§ØªØµØ§Ù„ Ø¨Ø§Ù„Ø®Ø§Ø¯Ù…" }
    }
  }, [])
