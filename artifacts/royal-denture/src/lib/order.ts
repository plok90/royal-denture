...
/** Opens WhatsApp with the order and persists it to Firestore. Shared by home form + cart drawer. */
export async function placeOrder(opts: {
  ...
  const { products, items, name, phone, notes, whatsappNumber } = opts
  const lines = items.map((item, i) => {
    const p = products.find((x) => x.id === item.productId || x.slug === item.productId)
    if (!p) return null
    return `${i + 1}. ${p.name_ar || p.name} × ${item.quantity} — ${(p.price * item.quantity).toLocaleString("ar-IQ")} د.ع`
  ...
  const digits = (whatsappNumber || "9647766463735").replace(/[^\d]/g, "")
  window.open(`https://wa.me/${digits}?text=${encodeURIComponent(msg)}`, "_blank")

  const orderItems = items.map((i) => {
    const p = products.find((x) => x.id === i.productId || x.slug === i.productId)!
    return {
  ...
  await saveOrder(name, phone, notes, orderItems, total)
}
...
export function buildStatusMessage(
  ...
  const itemList = items.map((item, i) => `${i + 1}. ${item.name_ar || item.name} × ${item.quantity}`).join("\n")
  const totalStr = total.toLocaleString("ar-IQ")

  if (status === "قيد المعالجة") {
    return `🔔 مرحباً ${name}،\n\nطلبكم في Royal Denture بدأ العمل عليه الآن ⚙️\n\n${itemList}\n\nالإجمالي: ${totalStr} د.ع\n\nسنعلمكم حين يكون جاهزاً 🙏`
  }
  if (status === "تم") {
    return `🏆 عزيزي ${name}،\n\nتم اكتمال وتسليم طلبكم في Royal Denture ✅\n\n${itemList}\n\nالإجمالي: ${totalStr} د.ع\n\nشكراً لتعاملكم معنا 🙏`
  }
  return null
}
...
export async function saveOrder(
  ...
  if (isReady()) {
    try { existingIds = await collectAllCaseIds() } catch { /* ignore */ }
    try {
      const s = await readSetting(SETTINGS_KEY)
      if (s) {
        const fallback = JSON.parse(s) as Order[]
  ...
  if (isReady()) {
    try {
      await insertOrderDoc(order as unknown as { id: string } & Record<string, unknown>)
      saveOrderToLocal(order)
      return
    } catch (e) {
      console.warn("orders insert:", e)
      const saved = await saveOrderToSettingsFallback(order)
...
async function saveOrderToSettingsFallback(order: Order): Promise<boolean> {
  try {
    const raw = await readSetting(SETTINGS_KEY)
    const orders: Order[] = raw ? JSON.parse(raw) : []
    orders.unshift(order)
    await upsertSetting(SETTINGS_KEY, JSON.stringify(orders))
    return true
  } catch { return false }
}
...
export async function getSettingsOrders(): Promise<Order[]> {
  ...
  try {
    ...
    const orders: Order[] = raw ? JSON.parse(raw) : []
    let changed = false
    const existing = orders.map(o => o.case_id).filter(Boolean) as string[]
...
export function getLocalOrders(): Order[] {
  try {
    ...
    let changed = false
    orders = orders.map(o => {
      if (o.status === "جديد") { changed = true; return { ...o, status: "قيد المعالجة", case_id: o.case_id || generateCaseId(orders.filter(x => x.id !== o.id).map(x => x.case_id).filter(Boolean) as string[]) } }
      if (!o.case_id) { changed = true; return { ...o, case_id: generateCaseId(orders.filter(x => x.id !== o.id).map(x => x.case_id).filter(Boolean) as string[]) } }
      return o
    })
...
export async function removeSettingsOrder(id: string): Promise<void> {
  ...
  try {
    const raw = await readSetting(SETTINGS_KEY)
    if (!raw) return
    const orders: Order[] = JSON.parse(raw).filter((o: Order) => o.id !== id)
    await upsertSetting(SETTINGS_KEY, JSON.stringify(orders))
  } catch { /* ignore */ }
}
...
export async function updateSettingsOrderStatus(id: string, status: string): Promise<void> {
  ...
  try {
    const raw = await readSetting(SETTINGS_KEY)
    if (!raw) return
    const orders: Order[] = JSON.parse(raw).map((o: Order) =>
...
export async function migrateOrdersCaseIds(): Promise<void> {
  if (!isReady()) return
  try {
    await migrateMissingCaseIds(generateCaseId)
  } catch { /* ignore */ }
}
  ...
  for (const o of orders) {
    ...
    if (products && Array.isArray(o.items)) {
      for (const item of o.items) {
        let p = products.find((pp: any) => pp.id === item.product_id || pp.slug === item.product_id)
