  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative w-full" style={{ minHeight: 200, maxHeight: "55vh", background: "linear-gradient(to bottom, rgba(0,0,0,0.3), rgba(0,0,0,0.1), rgba(0,0,0,0.3))", display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
          <img
            src={product.image_url}
            alt={product.name}
            onError={(e) => { const t = e.target as HTMLImageElement; if (!t.dataset.err) { t.dataset.err = "1"; t.src = FALLBACK_IMG; } }}
            style={{ maxHeight: "55vh", objectFit: "contain", borderRadius: 8, filter: "drop-shadow(0 8px 32px rgba(0,0,0,0.3))", maxWidth: "100%" }}
          />
        </div>
        ...
        <div className="p-5 sm:p-7 text-right" dir="rtl">
          <div className="flex items-start justify-between gap-4 mb-3">
            <div className="min-w-0">
              <h2 className="text-lg sm:text-xl font-semibold mb-0.5 leading-tight truncate" style={{ color: textPrimary }}>
                {product.name}
              </h2>
              {product.name_ar && <p className="text-xs sm:text-sm opacity-80" style={{ color: textMuted }}>{product.name_ar}</p>}
            </div>
